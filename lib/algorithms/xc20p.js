...
import {_chacha20, _encrypt, decrypt as _decrypt} from './c20p.js';
// FIXME: remove `xchacha20poly1305` lib
...
// FIXME: remove
export async function encrypt_old({data, additionalData, cek}) {
  if(!(data instanceof Uint8Array)) {
    throw new TypeError('"data" must be a Uint8Array.');
  }
  if(!(cek instanceof Uint8Array)) {
...
// FIXME: remove
export async function decrypt_old({ciphertext, iv, tag, additionalData, cek}) {
  if(!(iv instanceof Uint8Array)) {
    throw new Error('Invalid or missing "iv".');
  }
  if(!(ciphertext instanceof Uint8Array)) {
...
async function _hchacha20({key, nonce}) {
  ...
  await _chacha20({key, nonce, src: NULL_DATA, dst});
