...
export async function kekFromEphemeralPeer({keyAgreementKey, epk}) {
  ...
  // export to multikey key for Web KMS transport
  const ephemeralPublicKey = await publicKey.export(
...
export async function kekFromStaticPeer({ephemeralKeyPair, staticPublicKey}) {
  ...
  const {publicKey: remotePublicKey} = await EcdsaMultikey.from(
...
export async function generateEphemeralKeyPair() {
  ...
  const epk = await keyPair.toJwk({keyPair, secretKey: true});
