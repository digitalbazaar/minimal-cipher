...
export async function _encrypt({data, additionalData, cek, iv}) {
  ...
  // counter breaking security; using XChaCha20Poly1305 instead
  // probabilistically eliminates chances of a collision as it has a 192-bit IV
  if(iv === undefined) {
    iv = webcrypto.getRandomValues(new Uint8Array(cipher.nonceLength));
  }
...
// internal function exported for reuse by XChaCha20Poly1305
export function _chacha20({key, nonce, src, dst}) {
  // FIXME: use node.js implementation
